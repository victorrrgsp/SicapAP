...
public class LoginController {
    ...
    @CrossOrigin
    @GetMapping(path = {"/teste/{certificado}"})
    public ResponseEntity<?> teste(@PathVariable String certificado) {
        ...
        try {
            ...
            String resposta = Login.getDesafio(certificado);
    ...
    public ResponseEntity<?> autenticar(@org.springframework.web.bind.annotation.RequestBody User user) {
        ...
        try {
            ...
            String postdata = "ipUsuario=127.0.0.1" + //getIp.getRequest().getRemoteAddr() +
            ...
            lista.forEach( res -> {
                user1.setUserName(respostaJson.get("validacaoAssinatura").get("dados").get("cpf").toString());
                user1.setCertificado(resposta);
                user1.getDateEnd().addHours(2);
                user1.setUnidadeGestora(new UnidadeGestora(((Object[])res)[1].toString(), ((Object[])res)[2].toString(),
                        Integer.parseInt(((Object[])res)[3].toString())));

                user1.setCargoByInteger(Integer.parseInt(((Object[])res)[4].toString()));
            });
            //System.out.println(cpf);

            getIp.getRequest().getSession().setAttribute("user", user1);

            return ResponseEntity.ok().body(user1.getUserName());
