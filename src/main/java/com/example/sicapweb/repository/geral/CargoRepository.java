...
public class CargoRepository extends DefaultRepository<Cargo, BigInteger> {
    ...
    public Cargo buscarCargoPorcodigo(String codigo) {
        List<Cargo> list = getEntityManager().createNativeQuery("select ed.* from Cargo ed join InfoRemessa i on ed.chave = i.chave and i.idUnidadeGestora='"+ redisConnect.getUser(super.request).getUnidadeGestora().getId()+"'"+
