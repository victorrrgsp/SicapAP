...
public class UnidadeGestoraRepository extends DefaultRepository<UnidadeGestora, String> {
    ...
    public List<UnidadeGestora> buscaTodasOutraUnidadeGestora() {
        List<UnidadeGestora> list = entityManager.createNativeQuery(
                "select * from UnidadeGestora where UnidadeGestora.id <> '"+ redisConnect.getUser(request).getUnidadeGestora().getId()+"' ORDER BY nome ASC"
                ,UnidadeGestora.class).getResultList();
        return list;
    }
    ...
    public Boolean EhUnidadeGestoraRpps(){
        try {
            return ( getEntityManager().createNativeQuery("select  1 from UnidadeGestoraRpps where cnpjRpps=:cnpj  ").setParameter("cnpj", redisConnect.getUser(super.getRequest()).getUnidadeGestora().getId() ).getResultList().size()>0) ;
