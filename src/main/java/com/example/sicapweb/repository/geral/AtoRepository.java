...
public class AtoRepository extends DefaultRepository<Ato, BigInteger> {
        ...
        long totalRegistros;
            List<Ato> list = getEntityManager()
                    .createNativeQuery("select a.* from Ato a " +
                            ...
                            + redisConnect.getUser(request).getUnidadeGestora().getId() + "' " + search + " ORDER BY " + campo, Ato.class)
    ...
    public Integer countAtos(String search) {
        Query query = getEntityManager().createNativeQuery("select count(1) from Ato a join InfoRemessa i on a.chave = i.chave where i.idUnidadeGestora= '"+ redisConnect.getUser(request).getUnidadeGestora().getId()+ "' "+search);
    ...
    public List<Ato> findAllAfter2021() {
        var query = getEntityManager().createNativeQuery(
                "select  id = MAX(id), cnpjUgPublicacao, dataPublicacao, numeroAto, tipoAto, " +
                        "veiculoPublicacao,max(a.chave) as chave " +
                        "from Ato a " +
                        "    join InfoRemessa info on a.chave = info.chave " +
                        "where info.idUnidadeGestora = '" + redisConnect.getUser(request).getUnidadeGestora().getId() + "'"+
