...
public class AssinarRemessaRepository extends DefaultRepository<String, String> {
        ...
        try {
            return (InfoRemessa) entityManager.createNativeQuery(
                    "select * from InfoRemessa i" +
                            ...
                            " and i.idUnidadeGestora = '" + redisConnect.getUser(super.request).getUnidadeGestora().getId() + "'", InfoRemessa.class).getSingleResult();
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public void insertArquivo() {
        ...
        query.setParameter("cpf", redisConnect.getUser(super.request).getCpf());
        query.executeUpdate();
    ...
    public void insertAdmAssinatura(String chave) {
        ...
        query.setParameter("assinatura", idAssinatura);
        query.setParameter("cargo", redisConnect.getUser(super.request).getCargo().getValor());
        query.executeUpdate();
        entityManager.flush();
    }
    ...
    public void insertUsuarioAplicacao(InfoRemessa infoRemessa) {
        ...
        query.setParameter("usuario", redisConnect.getUser(super.request).getCpf());
        query.setParameter("unidade", redisConnect.getUser(super.request).getUnidadeGestora().getId());
        query.setParameter("exercicio", infoRemessa.getExercicio());
        query.setParameter("remessa", infoRemessa.getRemessa());
        query.setParameter("cargo", redisConnect.getUser(super.request).getCargo().getValor());
    ...
    public    PaginacaoUtil<HashMap<String,Object>> getExtratoDadosRemessa(Pageable paginacaoFrontEnd  , Integer remessa, Integer exercicio, Integer idTabelaRemessa, String serachEncoded , boolean paraExportarExcell)   {
        ...
        try {
            ...
            queryTabelaRemessa.setParameter("remessa",remessa).setParameter("exercicio",exercicio).setParameter("ug",redisConnect.getUser(super.getRequest()).getUnidadeGestora().getId());

            long totalRegistros = (Integer) queryTabelaQuantidade
                    .setParameter("remessa",remessa)
                    .setParameter("exercicio",exercicio)
                    .setParameter("ug",redisConnect.getUser(super.getRequest()).getUnidadeGestora().getId()).getSingleResult();
