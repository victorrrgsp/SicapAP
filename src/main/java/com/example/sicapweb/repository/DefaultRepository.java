...
//        }
    ...
    public boolean isRPPS(){
        return entityManager.createNativeQuery("select * from UnidadeGestoraRpps where cnpjRpps like '%"+redisConnect.getUser(getRequest()).getUnidadeGestora().getId()+"%'").getResultList().size() > 0;
    }
    ...
    public void deleteRestrito(BigInteger id) {
        try {
            ...
            objeto.setId(id);
            if(objeto.getChave().getIdUnidadeGestora().equals(redisConnect.getUser(request).getUnidadeGestora().getId())){
    ...
    public List<T> findAll() {
        ...
        return getEntityManager()
                .createQuery("select a from " + entityClass.getSimpleName() +
                        ...
                        + redisConnect.getUser(request).getUnidadeGestora().getId() + "'", entityClass)
                .getResultList();
    }
    ...
    public PaginacaoUtil<T> buscaPaginada(Pageable pageable, String searchParams, Integer tipoParams) {
        ...
        var query = getEntityManager()
                .createNativeQuery("select a.* from " + entityClass.getSimpleName() + " a " +
                        ...
                        + redisConnect.getUser(request).getUnidadeGestora().getId() + "' " + search + " ORDER BY " + campo, entityClass)
    ...
    public Integer count() {
        Query query = getEntityManager().createNativeQuery("select count(*) from " + entityClass.getSimpleName()
                + " a join InfoRemessa i on a.chave = i.chave where i.idUnidadeGestora= '"+ redisConnect.getUser(request).getUnidadeGestora().getId()+ "'");
        return (Integer) query.getSingleResult();
    }

    public InfoRemessa buscarPrimeiraRemessa() {

        var user = redisConnect.getUser(request);
        if(user == null){
        ...
        List<InfoRemessa> list = getEntityManager().createNativeQuery("select * from infoRemessa " +
                "where remessa = 1 and exercicio = 2021 and idUnidadeGestora = '" + user.getUnidadeGestora().getId() + "'", InfoRemessa.class).getResultList();
        return list.get(0);
    }
