...
public class AdmEnvioAssinaturaRepository extends DefaultRepository<AdmEnvioAssinatura, BigInteger> {
    ...
    public Integer gerarProtocolo() {
        Query query = getEntityManager().createNativeQuery("" +
                "SELECT coalesce(MAX(ID_PROTOCOLO), 0)+1 as idprotocolo from SICAPAP21W..PROTOCOLOS where ANO = YEAR(GETDATE())");
    ...
    public void salvarAndamentoProcesso(Map<String, Object> processo) {
        Query query = getEntityManager().createNativeQuery(
                "INSERT INTO SICAPAP21W..ProcessoAndamento([NumProc],[AnoProc],[Descricao],[DHinsert],[IdDepto]) " +
    ...
    public void salvarPessoasInteressadas(Map<String, Object> interessado) {
        Query query = getEntityManager().createNativeQuery(
                "INSERT INTO SICAPAP21W..PESSOAS_PROCESSO (NUM_PROC, ANO_PROC, ID_PESSOA, ID_PAPEL, ID_CARGO) " +
    ...
    public void salvarDocumento(Map<String, Object> processo) {
        Query query = getEntityManager().createNativeQuery(
                "INSERT INTO SICAPAP21W..[document]( [docmt_tipo], [dcnproc_pnumero], [dcnproc_pano], [docmt_numero], [docmt_ano], [docmt_depto] " +
    ...
    public Integer buscarUltimoIdDocumento() {
        Query query = getEntityManager().createNativeQuery(
                "select MAX(docmt_id) as id_documento from SICAPAP21W..[document]");
        return (Integer) query.getSingleResult();
    }

    public void salvarArquivosDocumentos(Map<String, Object> arquivo, Integer idDocumento) {
       Query query = getEntityManager().createNativeQuery(
               "insert into SICAPAP21W..DOCUMENT_ARQUIVOS (ID_DOCUMENT,NOME_ARQ,DESCRICAO,DATA,LOGIN_INSERIU,EXCLUIDO,UUID_CAS) " +
