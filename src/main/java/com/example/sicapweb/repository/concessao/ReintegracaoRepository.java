...
public class ReintegracaoRepository extends DefaultRepository<Reintegracao, BigInteger> {
    ...
    public PaginacaoUtil<Reintegracao> buscaPaginadaReintegracao(Pageable pageable, String searchParams, Integer tipoParams) {
        ...
        List<Reintegracao> list = getEntityManager()
                .createNativeQuery("select a.* from Reintegracao a " +
                        ...
                        "where i.idUnidadeGestora = '" + User.getUser().getUnidadeGestora().getId() + "' " + search + " ORDER BY " + campo, Reintegracao.class)
                .setFirstResult(pagina)
                .setMaxResults(tamanho)
                .getResultList();
        long totalRegistros = count();
        long totalPaginas = (totalRegistros + (tamanho - 1)) / tamanho;
        return new PaginacaoUtil<Reintegracao>(tamanho, pagina, totalPaginas, totalRegistros, list);
    }
    ...
    public List<Reintegracao> buscarReintegracao() {
        return getEntityManager().createNativeQuery(
                "select a.* from Reintegracao a " +
                        "join InfoRemessa i on a.chave = i.chave " +
                        "where i.idUnidadeGestora = '" + User.getUser().getUnidadeGestora().getId() + "'", Reintegracao.class)
                .getResultList();
    }
}
